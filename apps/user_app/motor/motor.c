...
void motor_init(void)
{
    ...
    mcpwm_init_param.pwm_ch_num = pwm_ch0;                // 通道号
    mcpwm_init_param.frequency = 10000;                   // 1KHz
    mcpwm_init_param.duty = 0;                            // 上电输出0%占空比
    ...
    mcpwm_init_param.pwm_aligned_mode = pwm_edge_aligned; // 边沿对齐
    mcpwm_init_param.pwm_ch_num = pwm_ch1;                // 通道号
    mcpwm_init_param.frequency = 10000;                   // 1KHz
    mcpwm_init_param.duty = 0;                            // 上电输出0%占空比
    mcpwm_init_param.h_pin = MOTOR_FALL_PIN;              // 任意引脚
    mcpwm_init_param.l_pin = -1; // 任意引脚,不需要就填-1
    mcpwm_init_param.complementary_en =
        0; // 两个引脚的波形, 0: 同步,  1: 互补，互补波形的占空比体现在H引脚上
    mcpwm_init(&mcpwm_init_param);
    ...
    // 初始化完成后，默认关闭电机
    gpio_set_output_value(MOTOR_PWR_PIN, MOTOR_OFF_LEV);
...
{
    gpio_set_output_value(MOTOR_PWR_PIN, MOTOR_ON_LEV);
}
...
{
    motor_stop();
    mcpwm_set_duty(pwm_ch0, (u16)((u32)10000 * 10 / 100));
}

void motor_fall(void)
{
    motor_stop();
    mcpwm_set_duty(pwm_ch1, (u16)((u32)10000 * 10 / 100));
}

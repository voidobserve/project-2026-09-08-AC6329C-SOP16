...
typedef struct
{
    ...
    opt_handle_t operate_handle_table
        [SUPPORT_OPT_HANDLE_MAX]; // 记录需要read,write,notify,indicate的ATT handle
...
static bool __check_device_is_match(u8 event_type, u8 info_type, u8 *data,
                                    int size,
                                    client_match_cfg_t **output_match_devices);
...
static bool __resolve_ext_adv_report(le_ext_adv_report_evt_t *evt, u16 len)
{
    ...
    if (__check_device_is_match((u8)(evt->Event_Type.event_type),
                                CLI_CREAT_BY_ADDRESS, evt->Address, 6,
                                &match_cfg)) {
        find_remoter = 1;
...
{
    bool find_remoter;
    u8 Data_Length = report_pt[GET_STRUCT_MEMBER_OFFSET(
        __periodic_adv_report_event, Data_Length)];
    u8 *Data =
        &report_pt[GET_STRUCT_MEMBER_OFFSET(__periodic_adv_report_event, Data)];

#if CHAIN_DATA_TEST_EN
    log_info("\n********* Sync periodic adv succ ***********\n");
    log_info_hexdump(Data, Data_Length);
#else
    find_remoter = __resolve_adv_report(Data_Length, Data);
 ...
 */
/*************************************************************************************************/
static int __gatt_client_event_callback_handler(int event, u8 *packet, u16 size,
                                                u8 *ext_param)
{
    if (__this->client_config->event_packet_handler) {

        return __this->client_config->event_packet_handler(event, packet, size,
                                                           ext_param);
    }
 ...
 */
/*************************************************************************************************/
static void __gatt_client_set_work_state(u16 conn_handle, ble_state_e state,
                                         u8 report_en)
{
    u8 packet_buf[4];
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_CLIENT_STATE,
                                         packet_buf, 3, 0);
}
...
static void __gatt_client_timeout_handler(int type_id)
{
    ...
    if (TO_TYPE_CREAT_CONN == type_id) {
        if (ble_gatt_client_get_work_state() == BLE_ST_CREATE_CONN) {
            ...
            __gatt_client_event_callback_handler(
                GATT_COMM_EVENT_CREAT_CONN_TIMEOUT, 0, 0, 0);
...
static void __gatt_client_timeout_add(int type_id, u32 set_ms)
{
    ...
    __this->client_timeout_id =
        sys_timeout_add((void *)type_id, __gatt_client_timeout_handler, set_ms);
}
...
int l2cap_connection_update_request_just(u8 *packet, hci_con_handle_t handle)
{
    log_info("slave request conn_update:\n-conn_handle= %04x\n-interval_min= "
             ...
             little_endian_read_16(packet, 2), little_endian_read_16(packet, 4),
...
void user_client_report_data_callback(att_data_report_t *report_data)
{
    if (report_data->conn_handle != __this->just_search_handle) {
        if (INVAIL_INDEX == ble_comm_dev_get_index(report_data->conn_handle,
                                                   GATT_ROLE_CLIENT)) {
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_DATA_REPORT,
                                         report_data, sizeof(att_data_report_t),
                                         0);
}
...
static void __do_operate_search_handle(void)
{
    ...
    if (!__this->gatt_search_config ||
        0 == __this->gatt_search_config->search_uuid_count) {
        return;
    }
    ...
    for (i = 0; i < __this->opt_handle_used_cnt; i++) {
        ...
        log_info("do opt:service_uuid16:%04x,charactc_uuid16:%04x\n",
                 opt_hdl_pt->search_uuid->services_uuid16,
                 opt_hdl_pt->search_uuid->characteristic_uuid16);
        ...
        if (cur_opt_type & ATT_PROPERTY_NOTIFY) {
            if (__this->gatt_search_config->auto_enable_ccc) {
                ...
                ble_comm_att_send_data(__this->client_search_handle,
                                       opt_hdl_pt->value_handle + 1, &tmp_16, 2,
                                       ATT_OP_WRITE);
                if (0x2a4d == opt_hdl_pt->search_uuid->characteristic_uuid16 &&
                    opt_hdl_pt->search_uuid->read_report_reference) {
                    tmp_16 = 0x55A1; // fixed
                    log_info("read_desc01:%04x\n",
                             opt_hdl_pt->value_handle + 1);
                    ble_comm_att_send_data(__this->client_search_handle,
                                           opt_hdl_pt->value_handle + 1,
                                           &tmp_16, 2, ATT_OP_READ);
            ...
            if (opt_hdl_pt->search_uuid->read_long_enable) {
                ...
                ble_comm_att_send_data(__this->client_search_handle,
                                       opt_hdl_pt->value_handle, &tmp_16, 2,
                                       ATT_OP_READ_LONG);
            } else {
                tmp_16 = 0x55A1; // fixed
                log_info("read:%04x\n", opt_hdl_pt->value_handle);
...
static void __check_target_uuid_match(search_result_t *result_info)
{
    ...
    for (i = 0; i < __this->gatt_search_config->search_uuid_count; i++) {
        ...
        if (result_info->services.uuid16) {
            ...
            if (memcmp(result_info->services.uuid128, t_uuid->services_uuid128,
                       16)) {
                /* log_info("b2"); */
                continue;
            }
        ...
        if (result_info->characteristic.uuid16) {
            if (result_info->characteristic.uuid16 !=
                t_uuid->characteristic_uuid16) {
                /* log_info("b3"); */
                /* log_info("%d: %04x--%04x",result_info->characteristic.uuid16,t_uuid->characteristic_uuid16); */
                continue;
            }
        } else {
            if (memcmp(result_info->characteristic.uuid128,
                       t_uuid->characteristic_uuid128, 16)) {
                /* log_info("b4"); */
                continue;
            }
        }
    ...
    if ((t_uuid->opt_type & result_info->characteristic.properties) !=
        t_uuid->opt_type) {
    ...
    opt_handle_t *opt_get =
        &__this->operate_handle_table[__this->opt_handle_used_cnt++];
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_SEARCH_MATCH_UUID,
                                         opt_get, sizeof(opt_handle_t), 0);
}
 ...
 */
/*************************************************************************************************/
void user_client_report_descriptor_result(
    charact_descriptor_t *result_descriptor)
{
    ...
    __gatt_client_event_callback_handler(
        GATT_COMM_EVENT_GATT_SEARCH_DESCRIPTOR_RESULT,
        &__this->client_search_handle, 2, result_descriptor);
}
...
void user_client_report_search_result(search_result_t *result_info)
{
    if (result_info == (void *)-1) {
        ...
        ble_comm_dev_set_handle_state(__this->client_search_handle,
                                      GATT_ROLE_CLIENT, BLE_ST_SEARCH_COMPLETE);
        __do_operate_search_handle();
        __gatt_client_set_work_state(__this->client_search_handle,
                                     BLE_ST_SEARCH_COMPLETE, 1);
        __gatt_client_event_callback_handler(
            GATT_COMM_EVENT_GATT_SEARCH_PROFILE_COMPLETE,
            &__this->client_search_handle, 2, 0);
    ...
    log_info(
        "{charactc, uuid16:%04x,index=%d,handle:%04x~%04x,value_handle=%04x}\n",
        result_info->characteristic.uuid16, result_info->characteristic_index,
        result_info->characteristic.start_handle,
        result_info->characteristic.end_handle,
...
static void __gatt_client_search_profile_start(void)
{
    ...
    __gatt_client_event_callback_handler(
        GATT_COMM_EVENT_GATT_SEARCH_PROFILE_START,
        &__this->client_search_handle, 2, 0);

    user_client_init(__this->client_search_handle, search_ram_buffer,
                     SEARCH_PROFILE_BUFSIZE);
...
int ble_gatt_client_scan_enable(u32 en)
{
    ...
    if (en) {
        ...
        if (__this->scan_conn_config->set_local_addr_tag ==
            USE_SET_LOCAL_ADDRESS_TAG) {
            le_controller_set_mac(
                &__this->scan_conn_config->local_address_info[1]);
        }

        log_info("scan_param: %d-%d-%d\n", __this->scan_conn_config->scan_type,
                 __this->scan_conn_config->scan_interval,
                 __this->scan_conn_config->scan_window);
        ble_op_set_scan_param(__this->scan_conn_config->scan_type,
                              __this->scan_conn_config->scan_interval,
                              __this->scan_conn_config->scan_window);
    }
    log_info("scan_en:%d\n", en);
                                    ...
                                    client_match_cfg_t **output_match_devices)
{
    ...
    if (!__this->gatt_search_config ||
        !__this->gatt_search_config->match_devices_count) {
        return false;
    }
    ...
    for (i = 0; i < __this->gatt_search_config->match_devices_count; i++) {
        ...
        if (cfg->create_conn_mode == conn_mode &&
            size == cfg->compare_data_len) {
            /* log_info("dev check\n"); */
...
static bool __resolve_adv_report(adv_report_t *report_pt, u16 len)
{
    ...
    if (__check_device_is_match(report_pt->event_type, CLI_CREAT_BY_ADDRESS,
                                report_pt->address, 6, &match_cfg)) {
        find_remoter = 1;
        log_info("catch mac ok\n");
        goto just_creat;
    }
    ...
    for (i = 0; i < report_pt->length;) {
        ...
        // if (adv_data_pt[0] == 0x55 && adv_data_pt[1] == 0xAA)
        // {
        //     printf_buf(report_pt->data, report_pt->length);
        // }
...
#define TEST_BOX_BLE_NAME_LEN 0xc
            if (0 ==
                memcmp(adv_data_pt, TEST_BOX_BLE_NAME, TEST_BOX_BLE_NAME_LEN)) {
            ...
            if (__check_device_is_match(report_pt->event_type,
                                        CLI_CREAT_BY_NAME, adv_data_pt,
                                        length - 1, &match_cfg)) {
                find_remoter = 1;
                log_info("catch name ok\n");
        ...
        case HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA:
            if (__check_device_is_match(report_pt->event_type, CLI_CREAT_BY_TAG,
                                        adv_data_pt, length - 1, &match_cfg)) {
                log_info("get_tag_string!\n");
                find_remoter = 1;
            }
            break;
...
just_creat:
    if (find_remoter) {
        if (__this->gatt_search_config->match_rssi_enable &&
            report_pt->rssi < __this->gatt_search_config->match_rssi_value) {
        ...
        if (__gatt_client_event_callback_handler(GATT_COMM_EVENT_SCAN_DEV_MATCH,
                                                 packet_data, 9, match_cfg)) {
...
int ble_gatt_client_create_connection_request(u8 *address, u8 addr_type,
                                              int mode)
{
    ...
    case BLE_ST_CREATE_CONN:
        if (CONFIG_BT_GATT_CLIENT_NUM == 1) {
            ...
            break;
#else
            r_printf("Please wait Please wait for the first connection to "
                     "complete!!");
#endif
        }
    ...
    memcpy(create_conn_par, &create_default_param_table,
           sizeof(struct create_conn_param_ext_t));

    create_conn_par->conn_interval_min =
        __this->scan_conn_config->creat_conn_interval;
    create_conn_par->conn_interval_max =
        __this->scan_conn_config->creat_conn_interval;
    create_conn_par->conn_latency =
        __this->scan_conn_config->creat_conn_latency;
    create_conn_par->supervision_timeout =
        __this->scan_conn_config->creat_conn_super_timeout;
    memcpy(create_conn_par->peer_address, address, 6);
    create_conn_par->peer_address_type = addr_type;
    ...
    if (ret) {
        ...
        if (__this->scan_conn_config->creat_state_timeout_ms) {
            __gatt_client_timeout_add(
                TO_TYPE_CREAT_CONN,
                __this->scan_conn_config->creat_state_timeout_ms);
        }
    }
    return ret;
}
...
{
    if (ble_gatt_client_get_work_state() == BLE_ST_CREATE_CONN) {
        __gatt_client_set_work_state(INVAIL_CONN_HANDLE,
                                     BLE_ST_SEND_CREATE_CONN_CANNEL, 1);
...
static void __gatt_client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    if (!__this->gatt_search_config ||
        ...
        putchar('~');
        __gatt_client_event_callback_handler(GATT_COMM_EVENT_SCAN_ADV_REPORT,
                                             report_pt, len, 0);
        return;
    }
    ...
    if (find_tag && __this->scan_conn_config &&
        __this->scan_conn_config->creat_auto_do) {
        ble_gatt_client_scan_enable(0);
#if EXT_ADV_MODE_EN
        if (ble_gatt_client_create_connection_request(evt->Address,
                                                      evt->Address_Type, 0))
#else
        if (ble_gatt_client_create_connection_request(
                report_pt->address, report_pt->address_type, 0))
...
void ble_gatt_client_passkey_input(u32 *key, u16 conn_handle)
{
    ...
    *key = 123456; // default key
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_SM_PASSKEY_INPUT,
                                         tmp_val, 6, 0);
 ...
 */
/*************************************************************************************************/
void ble_gatt_client_sm_packet(uint8_t packet_type, uint16_t channel,
                               uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case SM_EVENT_JUST_WORKS_REQUEST:
            ...
            __gatt_client_event_callback_handler(
                GATT_COMM_EVENT_ENCRYPTION_REQUEST, &event->con_handle, 2,
                &__this->client_encrypt_process);
            break;
 ...
 */
/*************************************************************************************************/
void ble_gatt_client_cbk_packet_handler(uint8_t packet_type, uint16_t channel,
                                        uint8_t *packet, uint16_t size)
{
    u16 tmp_val[4];
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
                ...
                if (BT_OP_SUCCESS != packet[3]) {
                    ...
                    tmp_val[0] =
                        hci_subevent_le_enhanced_connection_complete_get_connection_handle(
                            packet);
                    log_info(
                        "HCI_SUBEVENT_LE_ENHANCED_CONNECTION_COMPLETE : %0x\n",
                        tmp_val[0]);
                    log_info(
                        "conn_interval = %d\n",
                    ...
                    __gatt_client_event_callback_handler(
                        GATT_COMM_EVENT_CONNECTION_COMPLETE, tmp_val, 2,
                        packet);
            ...
            case HCI_SUBEVENT_LE_PERIODIC_ADVERTISING_SYNC_ESTABLISHED:
                ...
                log_info(
                    "APP HCI_SUBEVENT_LE_PERIODIC_ADVERTISING_SYNC_ESTABLISHED "
                    ":0x%x 0x%x",
                    packet[3], sync_handle);
                break;

            case HCI_SUBEVENT_LE_PERIODIC_ADVERTISING_SYNC_LOST:
                sync_handle = little_endian_read_16(packet, 3);
                log_info("APP HCI_SUBEVENT_LE_PERIODIC_ADVERTISING_SYNC_LOST "
                         "sync_handle:0x%x",
                         sync_handle);
                break;
#endif /*PERIODIC_ADV_MODE_EN*/
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE: {
                ...
                tmp_val[0] =
                    hci_subevent_le_connection_complete_get_connection_handle(
                        packet);
                log_info("HCI_SUBEVENT_LE_CONNECTION_COMPLETE:conn_handle= "
                         "%04x,rssi= %d\n",
                ...
                __this->client_search_handle = tmp_val[0];
                ble_comm_dev_set_handle_state(__this->client_search_handle,
                                              GATT_ROLE_CLIENT, BLE_ST_CONNECT);
                __this->client_encrypt_process = LINK_ENCRYPTION_NULL;
                __gatt_client_set_work_state(INVAIL_CONN_HANDLE, BLE_ST_IDLE,
                                             0);
                __gatt_client_set_work_state(tmp_val[0], BLE_ST_CONNECT, 1);
                __gatt_client_event_callback_handler(
                    GATT_COMM_EVENT_CONNECTION_COMPLETE, tmp_val, 2, packet);
            ...
            case HCI_SUBEVENT_LE_DATA_LENGTH_CHANGE:
                ...
                log_info("TX_Octets:%d, Time:%d\n",
                         ...
                         little_endian_read_16(packet, 7));
                log_info("RX_Octets:%d, Time:%d\n",
                         little_endian_read_16(packet, 9),
                         little_endian_read_16(packet, 11));
                ...
                if (config_btctler_le_features & LE_2M_PHY) {
                    ...
                    if (config_btctler_coded_type == CONN_SET_PHY_OPTIONS_S2) {
                        log_info(">>>>>>>>s3--request CODED S2, %04x\n",
                                 tmp_val[0]);
                        ble_comm_set_connection_data_phy(
                            tmp_val[0], CONN_SET_CODED_PHY, CONN_SET_CODED_PHY,
                            CONN_SET_PHY_OPTIONS_S2);
                    } else {
                        ...
                        ble_comm_set_connection_data_phy(
                            tmp_val[0], CONN_SET_CODED_PHY, CONN_SET_CODED_PHY,
                            CONN_SET_PHY_OPTIONS_S8);
                    }
                }

                __gatt_client_event_callback_handler(
                    GATT_COMM_EVENT_CONNECTION_DATA_LENGTH_CHANGE, tmp_val, 2,
                    packet);
                break;
            ...
            case HCI_SUBEVENT_LE_PHY_UPDATE_COMPLETE:
                ...
                log_info(
                    "APP HCI_SUBEVENT_LE_PHY_UPDATE %s\n",
                    hci_event_le_meta_get_phy_update_complete_status(packet)
                        ? "Fail"
                        : "Succ");
                log_info("Tx PHY: %s\n",
                         client_phy_result
                             [hci_event_le_meta_get_phy_update_complete_tx_phy(
                                 packet)]);
                log_info("Rx PHY: %s\n",
                         client_phy_result
                             [hci_event_le_meta_get_phy_update_complete_rx_phy(
                                 packet)]);
                __gatt_client_event_callback_handler(
                    GATT_COMM_EVENT_CONNECTION_PHY_UPDATE_COMPLETE, tmp_val, 2,
                    packet);
                break;
            }
            break;
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            ...
            log_info("HCI_EVENT_DISCONNECTION_COMPLETE:conn_handle= %04x, "
                     "reason= %02x\n",
                     tmp_val[0], packet[5]);
            ble_comm_dev_set_handle_state(tmp_val[0], GATT_ROLE_CLIENT,
                                          BLE_ST_DISCONN);
            __gatt_client_set_work_state(tmp_val[0], BLE_ST_DISCONN, 1);
            __gatt_client_event_callback_handler(
                GATT_COMM_EVENT_DISCONNECT_COMPLETE, tmp_val, 4, packet);
            __gatt_client_check_auto_scan();
        } break;
        ...
        case ATT_EVENT_MTU_EXCHANGE_COMPLETE: {
            ...
            log_info("handle= %02x, ATT_MTU= %u,payload= %u\n", tmp_val[0],
                     tmp_val[1], tmp_val[1] - 3);
            __gatt_client_event_callback_handler(
                GATT_COMM_EVENT_MTU_EXCHANGE_COMPLETE, tmp_val, 4, 0);
        } break;
        ...
        case HCI_EVENT_ENCRYPTION_CHANGE: {
            ...
            __gatt_client_event_callback_handler(
                GATT_COMM_EVENT_ENCRYPTION_CHANGE, tmp_val, 4, 0);
            if (ble_comm_need_wait_encryption(GATT_ROLE_CLIENT)) {
                __gatt_client_search_profile_start();
            }
            ...
            if (config_le_sm_support_enable) {
                if (config_btctler_le_features &
                    ...
                    ble_comm_set_connection_data_length(
                        tmp_val[0], config_btctler_le_acl_packet_length, 2120);
                } else if (config_btctler_le_features & LE_2M_PHY) {
 ...
 */
/*************************************************************************************************/
int ble_gatt_client_connetion_update_set(
    u16 conn_handle, const struct conn_update_param_t *param)
{
    int ret;
